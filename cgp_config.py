...
# Evaluation of CNNs
def cnn_eval(net, gpu_id, epoch_num, batchsize, dataset, has_val, valid_data_ratio, verbose, size_1d):

    print('\tgpu_id:', gpu_id, ',', net)
    train = cnn.CNN_train(size_1d, dataset, has_val, validation=True, valid_data_ratio=valid_data_ratio, verbose=verbose)
...
class CNNEvaluation(object):
    def __init__(self, gpu_num, epoch_num=50, batchsize=256, dataset='cifar10', has_val=False, valid_data_ratio=0.1, verbose=True,size_1d=224):
    ...
    def __call__(self, net_lists):
        ...
        for i in np.arange(0, len(net_lists), self.gpu_num):
            ...
            arg_data = [(cnn_eval, net_lists[i+j], j, self.epoch_num, self.batchsize, self.dataset, self.has_val, 
